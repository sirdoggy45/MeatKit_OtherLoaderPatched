...
public static class SerializedPropertyExtensions
{
    ...
    public static object GetValue(this SerializedProperty property)
    {
        ...
        Debug.Log("Property Path: " + propertyPath);
        Debug.Log("Target Object: " + value.GetType());

        int i = 0;
        PropertyPathComponent token;
        while (NextPathComponent(propertyPath, ref i, out token))
    ...
    static object GetMemberValue(object container, string name)
    {
        ...
        for (int i = 0; i < members.Length; ++i)
        {
            ...
            {
                return ((FieldInfo)members[i]).GetValue(container);
            }
                
            else if (members[i] is PropertyInfo)
            {
                return ((PropertyInfo)members[i]).GetValue(container, null);
            }
    ...
    static void SetMemberValue(object container, string name, object value)
    {
        ...
        for (int i = 0; i < members.Length; ++i)
        {
            if (members[i] is FieldInfo)
            {
                ((FieldInfo)members[i]).SetValue(container, value);
                return;
            }
            else if (members[i] is PropertyInfo)
            {
                ((PropertyInfo)members[i]).SetValue(container, value, null);
                return;
            }
        }
