...
public class AdvancedPrefabLoader : EditorWindow
{
    ...
    {
        if (PrefabLoaderState.Instance.CurrentSpawnMode == PrefabLoaderSpawnMode.DeepCopy)
        {
            DrawDeepCopySettings();
        }
    }

    private void DrawDeepCopySettings()
    {
        PrefabLoaderState.Instance.RipMeshes = EditorGUILayout.Toggle("Rip Meshes On Spawn", PrefabLoaderState.Instance.RipMeshes);
    }
    ...
    {
        string previousAssetBundleFolder = Directory.GetParent(PrefabLoaderState.Instance.SelectedBundlePath).FullName;
